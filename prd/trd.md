...
## 7. 后端架构 (Backend Architecture)

### 7.1 微服务架构设计
- **API Gateway**: 基于 Kong 或 Traefik，处理身份验证、限流及请求分发。
- **Project Service**: 管理工程元数据（VEIR Schema），支持多版本快照与分支管理。
- **Asset Management Service (MAM)**: 负责素材上传、元数据提取（FFprobe）及生命周期管理。
- **Render Farm Manager**: 分布式渲染调度中心，管理渲染节点的任务分配与负载均衡。

### 7.2 媒体处理流水线 (Media Pipeline)
- **Transcoding Engine**: 基于 FFmpeg 的集群化转码，自动生成多码率 Proxy 素材。
- **Server-Side Rendering (SSR)**: 
  - 使用 Headless 环境运行 VEIR Core 渲染引擎，确保预览与导出的一致性（What You See Is What You Get）。
  - 支持分布式分段渲染（Chunk-based Rendering）后合并，提升导出速度。
- **Task Queue**: 使用 RabbitMQ 或 Kafka 处理高并发的转码与渲染异步任务。

### 7.3 数据存储策略
- **Metadata Storage**: MongoDB 或 PostgreSQL (JSONB) 存储非结构化的编辑轨道数据。
- **Object Storage**: 集成 AWS S3 或 阿里云 OSS，配合 CDN 加速素材分发。
- **Hot Data Cache**: Redis 存储活跃编辑会话与渲染中间件状态。

### 7.4 实时协作后端
- **WebSocket / WebRTC**: 实现多用户协同编辑时的状态同步。
- **Conflict Resolution**: 采用 OT (Operational Transformation) 或 CRDT (Conflict-free Replicated Data Types) 算法解决编辑冲突。

## 8. 部署与运维 (DevOps)
- **Containerization**: 全量业务容器化（Docker + Kubernetes）。
- **GPU Acceleration**: K8s 节点支持 NVIDIA Docker 插件，实现硬件加速转码。
- **Observability**: Prometheus + Grafana 监控渲染节点性能，ELK Stack 收集处理日志。