...
import { getDraft } from '@/features/editor-drafts/idb'
...
const exportPresets: ExportPreset[] = [
  { id: '4k', name: '4K 超清', icon: Monitor, resolution: '3840 × 2160', fps: 60, bitrate: '50 Mbps', estimatedSize: '~850 MB' },
  { id: '1080p', name: '1080P 高清', icon: Monitor, resolution: '1920 × 1080', fps: 60, bitrate: '20 Mbps', estimatedSize: '~320 MB' },
  { id: '720p', name: '720P 标清', icon: Tablet, resolution: '1280 × 720', fps: 30, bitrate: '8 Mbps', estimatedSize: '~130 MB' },
  { id: 'douyin', name: '抖音/快手', icon: Smartphone, resolution: '1080 × 1920', fps: 30, bitrate: '15 Mbps', estimatedSize: '~240 MB', platform: '竖屏短视频' },
  { id: 'youtube', name: 'YouTube', icon: Youtube, resolution: '1920 × 1080', fps: 60, bitrate: '25 Mbps', estimatedSize: '~400 MB', platform: 'YouTube 推荐' },
  { id: 'bilibili', name: 'B站', icon: Film, resolution: '1920 × 1080', fps: 60, bitrate: '16 Mbps', estimatedSize: '~260 MB', platform: 'B站推荐' },
]
...
export default function DraftExportPage({ params }: { params: { draftId: string } }) {
  ...
  const startExport = useCallback(async () => {
    ...
    if (!projectToExport) {
      ...
      setExportMessage('未找到草稿，使用示例项目演示...')
    }
  ...
  useEffect(() => {
    return () => {
      if (compositionResult?.downloadUrl) URL.revokeObjectURL(compositionResult.downloadUrl)
    }
  }, [compositionResult])
    ...
    <div className="flex-1 flex min-h-0 overflow-hidden">
      <div className="flex-1 flex flex-col p-6 border-r border-surface-800">
        ...
        <div className="flex-1 flex items-center justify-center mb-6">
          ...
          >
            {compositionResult ? (
              <video ref={videoRef} src={compositionResult.downloadUrl} controls className="w-full h-full object-contain" />
            ) : (
          ...
          {error && (
            <motion.div key="error" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
              <Card variant="glass" className="p-6 border-red-500/30">
                ...
                <Button variant="outline" onClick={resetExport}>重试</Button>
              </Card>
            </motion.div>
          )}

          {isExporting && !error && (
            <motion.div key="exporting" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
              <Card variant="glass" className="p-6">
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-surface-400">{getStageLabel(exportStage)} · {exportMessage}</p>
                  </div>
                  <span className="text-2xl font-mono font-bold text-amber-400">{Math.round(exportProgress)}%</span>
                </div>
            ...
            <motion.div key="complete" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}>
              <Card variant="glass" className="p-6 border-success/30">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div className="flex-1">
                    <h3 className="text-lg font-semibold text-surface-100 mb-1">🎉 导出成功！</h3>
                    <p className="text-surface-400">视频已生成，可以直接下载或分享</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-3 mb-6">
                  <Card className="p-3">
                    ...
                    <p className="text-lg font-semibold text-surface-100">{formatDuration(compositionResult.duration)}</p>
                  </Card>
                  <Card className="p-3">
                    <p className="text-sm text-surface-500 mb-1">文件大小</p>
                    <p className="text-lg font-semibold text-surface-100">{formatBytes(compositionResult.size)}</p>
                  </Card>
                  <Card className="p-3">
                    <p className="text-sm text-surface-500 mb-1">格式</p>
                    <p className="text-lg font-semibold text-surface-100 uppercase">{compositionResult.format}</p>
                  </Card>
                </div>

                <div className="flex gap-3">
                  <Button variant="primary" size="lg" leftIcon={<Download className="w-5 h-5" />} className="flex-1" onClick={handleDownload}>
                    下载视频
                  </Button>
                  <Button variant="outline" size="lg" leftIcon={<Zap className="w-5 h-5" />}>分享</Button>
                  <Button variant="ghost" size="lg" leftIcon={<ExternalLink className="w-5 h-5" />}>复制链接</Button>
                </div>
              </Card>
            </motion.div>
          )}
      ...
      <div className="w-96 p-6 bg-surface-900/50 overflow-y-auto">
        <h2 className="text-lg font-semibold text-surface-100 mb-4">导出设置</h2>
              ...
              return (
                <button
                  ...
                  className={`
                    ...
                    ${isSelected ? 'bg-amber-400/20 border border-amber-400/50' : 'bg-surface-800 border border-surface-700 hover:border-surface-600'}
                    ${isExporting ? 'opacity-50 cursor-not-allowed' : ''}
                  `}
                >
                  <div className="flex items-center gap-2 mb-1">
                    <Icon className={`w-4 h-4 ${isSelected ? 'text-amber-400' : 'text-surface-400'}`} />
                    <span className={`text-sm font-medium ${isSelected ? 'text-amber-400' : 'text-surface-200'}`}>{p.name}</span>
                  </div>
        ...
        <Card className="p-4 mb-6">
          <h3 className="text-sm font-medium text-surface-200 mb-3 flex items-center gap-2">
            <Settings className="w-4 h-4" /> 导出参数
          </h3>
        ...
        <div className="mb-6">
          ...
          <Slider value={[quality]} onValueChange={(v) => setQuality(v[0])} max={100} min={50} disabled={isExporting} />
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Switch checked={includeSubtitles} onCheckedChange={setIncludeSubtitles} disabled={isExporting} />
          </div>
          <div className="flex items-center justify-between">
            <div>
            ...
            <Switch checked={includeWatermark} onCheckedChange={setIncludeWatermark} disabled={isExporting} />
          </div>
        </div>
        ...
        <Card className="p-4 mt-6 bg-amber-400/5 border-amber-400/20">
          <div className="flex gap-3">
            ...
            <div>
              <p className="text-sm text-surface-200 mb-1">提示</p>
              <p className="text-xs text-surface-400">
                导出将使用项目分辨率 ({projectResolution[0]}×{projectResolution[1]})。首次合成可能需要较长时间加载资源。
              </p>
            </div>
          </div>
        </Card>
